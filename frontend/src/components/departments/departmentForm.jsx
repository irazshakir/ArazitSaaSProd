...
const DepartmentForm = ({ 
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log('Submitting department with data:', submitData);
      ...
      const endpoints = isEditMode 
        ? [
            ...
            `auth/departments/${initialData.id}/`
          ]
        : [
            'departments/',
            'users/departments/',
      ...
      // Success message
      if (isEditMode) {
        message.success(`Department "${formData.name}" updated successfully!`);
      } else {
        message.success(`Department "${formData.name}" created successfully!`);
      }
      ...
      >
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', mb: 3 }}>
          ...
          <Typography variant="h6" component="h1" sx={{ fontWeight: 600 }}>
            {isEditMode ? `Edit Department: ${initialData.name}` : 'Create New Department'}
          </Typography>
        </Box>
            ...
            <Grid item xs={12} sx={{ mt: 2 }}>
              <Box sx={{ display: 'flex', justifyContent: 'flex-end', gap: 2 }}>
                ...
                <Button
                  ...
                  disabled={loading}
                >
                  {loading ? 'Saving...' : (isEditMode ? 'Save Changes' : 'Create Department')}
                </Button>
              </Box>
            </Grid>
          </Grid>
